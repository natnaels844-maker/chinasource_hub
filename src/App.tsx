...
function Pagination({page,total,setPage}:{page:number;total:number;setPage:(n:number)=>void}){if(total<=1)return null;const start=Math.max(1,Math.min(total-6,page-3));const nums=Array.from({length:Math.min(7,total)},(_,i)=>start+i);return <div style={{display:'flex',justifyContent:'center',gap:8,marginTop:30,flexWrap:'wrap'}}><button className="secondary" disabled={page===1} onClick={()=>setPage(page-1)}>← Prev</button>{nums.map(n=><button key={n} className={n===page?'primary':'secondary'} onClick={()=>setPage(n)}>{n}</button>)}<button className="secondary" disabled={page===total} onClick={()=>setPage(page+1)}>Next →</button></div>}
function Modal({p,info,close,checkout,currency}:{p:Product;info:SupplierInfo|null;close:()=>void;checkout:(p:Product)=>void;currency:Currency}){return <div className="overlay" onClick={close}><div className="modal" onClick={e=>e.stopPropagation()}><button className="x" onClick={close}>×</button><img src={p.image} alt={p.name}/><div className="modalText">{p.bigDeal&&<div className="dealBadge inline">🔥 BIG DEAL</div>}<span className="tag">{p.source} · {p.category}</span><h2>{p.name}</h2><p>Supplier-listed product from the imported Alibaba catalog.</p><div className="meta" style={{marginBottom:18}}><b>{priceInCurrency(p.price,currency)}</b><span>{p.moq}</span></div><div className="supplierBox">{info?<><b>✓ Supplier access unlocked</b><p><strong>{info.supplier}</strong><br/>{info.address}</p><a href={info.url} target="_blank" rel="noreferrer">Open original Alibaba listing →</a></>:<><b>Supplier identity protected</b><p>Supplier name, address and direct listing are hidden until your payment is verified.</p><button className="primary full" onClick={()=>checkout(p)}>Pay $2.50 USD · Unlock Supplier</button><small>Secure payment powered by Stripe.</small></>}</div></div></div></div>}
function Services(){return <main className="page"><div className="pageHead"><span className="eyebrow">SERVICES</span><h1>From product discovery to supplier access.</h1><p>We help buyers identify China sourcing opportunities, compare supplier listings, and access the original supplier listing after payment verification.</p></div><div className="serviceGrid"><div className="service"><span>01</span><h3>Product discovery</h3><p>Browse the imported catalog and search by product name or category.</p></div><div className="service"><span>02</span><h3>Supplier research</h3><p>We keep supplier identity private until the buyer chooses to unlock it.</p></div><div className="service"><span>03</span><h3>Direct access</h3><p>After verified payment, the original Alibaba listing is returned.</p></div><div className="service"><span>04</span><h3>Custom sourcing</h3><p>Need something not listed? Request a sourcing search for a specific product.</p></div></div></main>}
function How(){return <main className="page"><div className="pageHead"><span className="eyebrow">HOW IT WORKS</span><h1>Browse → Evaluate → Pay → Buy direct</h1><p>Supplier details are not exposed in the public catalog.</p></div><div className="steps"><div className="step"><b>1</b><h3>Browse</h3><p>Search the 240 imported Alibaba listings.</p></div><div className="step"><b>2</b><h3>Evaluate</h3><p>Review the product, displayed price, MOQ and source information.</p></div><div className="step"><b>3</b><h3>Unlock</h3><p>Pay $2.50 securely through Stripe.</p></div><div className="step"><b>4</b><h3>Buy direct</h3><p>Once payment is verified, supplier information and the original listing link are shown.</p></div></div></main>}
function Request(){return <main className="page"><div className="pageHead"><span className="eyebrow">REQUEST A QUOTE</span><h1>Tell us what you want to source.</h1><p>Describe the product, quantity, target price and destination. We can research suitable Chinese supplier listings.</p></div><div className="form"><label>Product name<input placeholder="e.g. rechargeable LED lamps"/></label><label>Quantity<input placeholder="e.g. 500 units"/></label><label>Target price<input placeholder="e.g. under $8 per unit"/></label><label>Destination<input placeholder="Country / city"/></label><label className="wide">Requirements<textarea rows={6} placeholder="Specifications, packaging, branding, certifications, etc."/></label><button className="primary" type="button" onClick={()=>alert('Request form ready — connect it to your email or CRM when you are ready.')}>Submit sourcing request</button></div></main>}
